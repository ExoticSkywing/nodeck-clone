...
- iOS 16.3 hardware-silent/media-route fallback: a WebKit-only attached `<audio>` pool uses MP3 equivalents for all 22 one-shots and starts them from trusted `touchend`/`pointerup`; Reynard/Gecko and desktop remain on the source Howler owner. This is a physical-device test candidate because automation can verify `HTMLMediaElement.play()`/`playing`, not human audibility.
